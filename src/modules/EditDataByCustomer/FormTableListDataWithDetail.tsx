...
interface FormTableListDataWithDetailProps {
  ...
  onCheck: (items: StateListWithDetail[]) => void
}
...
const FormTableListDataWithDetail: FunctionComponent<FormTableListDataWithDetailProps> = ({
  ...
  const [items, setItems] = useState<StateListWithDetail[]>([])
  ...
  const handleSavePDF = () => {
    ...
    html2canvas(source, options).then((canvas) => {
      ...
      doc.save(`DataWithDetailByCustomer_${moment().format('DD-MMM-YYYY')}.pdf`)
      setIsPDF(false)
    })
  }
  ...
  const handleChange = (key: keyof StateListWithDetail, id: number, val: any) => {
    items[id] = {
      ...items[id],
      [key]: val,
      error: {
  ...
  return (
    <>
      <div style={isPDF ? { overflow: 'auto' } : undefined}>
        <div ref={tableListAndSumRef} style={isPDF ? { width: 2200 } : undefined}>
          <Grid>
            <Grid.Row columns={2}>
              <Grid.Column>
          ...
          <Segment.Group raised>
            <Wrapper>
              <Table celled textAlign='center' style={{ width: 2200 }}>
                <Table.Header>
                  <Table.Row>
                    {!isPDF &&
                      ...
                      <Table.Row key={item.id} warning={item.selected}>
                        ...
                        {item.editable &&
                          <>
                            ...
                            <Table.Cell>
                              <Field
                                placeholder='...'
                                value={item.receiveMonth}
                                onChange={(e, data) => handleChange('receiveMonth', index, data.value)}
                              />
                            </Table.Cell>
                            <Table.Cell>
                              <Field
                                ...
                                onChange={(e, data) => handleChange('productDescription', index, data.value)}
                              />
                            </Table.Cell>
                            <Table.Cell>
                              <Field
                                ...
                                placeholder='...'
                                value={item.quantity}
                                onChange={(e, data) => handleChange('quantity', index, parseFloat(data.value))}
                              />
                            </Table.Cell>
                            <Table.Cell>
                              <Field
                                placeholder='...'
                                value={item.unit}
                                onChange={(e, data) => handleChange('unit', index, data.value)}
                              />
                            </Table.Cell>
                            <Table.Cell>
                              <Field
                                ...
                                placeholder='...'
                                value={item.step}
                                onChange={(e, data) => handleChange('step', index, parseFloat(data.value))}
                              />
                            </Table.Cell>
                            <Table.Cell>
                              <Field
                                placeholder='...'
                                value={item.rate}
                                onChange={(e, data) => handleChange('rate', index, data.value)}
                              />
                            </Table.Cell>
                            <Table.Cell>
                              <Field
                        ...
                        {!item.editable &&
                          <>
                            ...
                            <Table.Cell error={item.error.receiveMonth}>{item.receiveMonth}</Table.Cell>
                            <Table.Cell error={item.error.productDescription}>{item.productDescription}</Table.Cell>
                            <Table.Cell error={item.error.quantity}>{item.quantity}</Table.Cell>
                            <Table.Cell error={item.error.unit}>{item.unit}</Table.Cell>
                            <Table.Cell error={item.error.step}>{item.step}</Table.Cell>
                            <Table.Cell error={item.error.rate}>{item.rate}</Table.Cell>
